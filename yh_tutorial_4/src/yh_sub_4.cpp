...
def msgCallback(msg):
    ...
    rospy.loginfo("receive msg-stamp.secs_4_sub_Last = %d", msg.stamp.secs)
    
    rospy.loginfo("receive msg-stamp.nsecs_4_sub_Last = %d", msg.stamp.nsecs)
    
    rospy.loginfo("receive msg-data_4_sub_Last = %d", msg.data)

def listener():
    rospy.init_node("yh_sub_4", anonymous=True)
    rospy.Subscriber("yh_topic_4_1", yh_msg_4, msgCallback)
...
void msgCallback(const yh_tutorial_4::yh_msg_4::ConstPtr& msg)   // 메시지 콜백함수, 서브스크라이버가 메시지를 수신했을 때 동작하는 함수이다.
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = nh.subscribe("yh_topic_4_1", 100, msgCallback);    // 서브스크라이버 선언
